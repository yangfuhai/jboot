...
public class JbootActionReporter {
	...
	public static final void report(String target, Controller controller, Action action) throws NotFoundException {
		CtClass ctClass = ClassPool.getDefault().get(action.getControllerClass().getCanonicalName());
		...
		if (inters.length > 0) {
			...
			for (int i=0; i<inters.length; i++) {
				...
				sb.append(ic.getName()).append(".(").append(ic.getSimpleName()).append(".java:"+lineNumber+")");
			}
			sb.append("\n");
		}
